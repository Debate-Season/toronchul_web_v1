"use client";

import { Suspense, useEffect, useRef, useState } from "react";
import { useRouter, useSearchParams } from "next/navigation";
import { loginWithOidc } from "@/lib/api/auth";
import useAuthStore from "@/store/useAuthStore";
import { verifyAppleOAuthState } from "@/lib/auth/apple";

export default function AppleCallbackPage() {
  return (
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center">
          <p className="text-body-16 text-text-secondary animate-pulse">
            Apple 로그인 처리 중...
          </p>
        </div>
      }
    >
      <AppleCallbackContent />
    </Suspense>
  );
}

function AppleCallbackContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [error, setError] = useState<string | null>(null);

  const { _hasHydrated, isLogin, setTokens, setProfileStatus, setTermsStatus } =
    useAuthStore();

  const processed = useRef(false);
  const cancelled = useRef(false);

  useEffect(() => {
    return () => { cancelled.current = true; };
  }, []);

  // 이미 로그인 상태면 홈으로
  useEffect(() => {
    if (!_hasHydrated) return;
    if (isLogin) router.replace("/");
  }, [_hasHydrated, isLogin, router]);

  useEffect(() => {
    if (!_hasHydrated) return;
    if (processed.current) return;
    processed.current = true;

    const idToken = searchParams.get("id_token");
    const state = searchParams.get("state");
    const errorParam = searchParams.get("error");

    // Apple 에서 에러로 돌아온 경우
    if (errorParam) {
      if (errorParam === "user_cancelled_authorize") {
        setError("Apple 인증이 취소되었습니다.");
      } else {
        setError(toUserMessage(errorParam));
      }
      return;
    }

    if (!idToken) {
      // id_token 없이 직접 접근 또는 새로고침 → 홈으로
      router.replace("/");
      return;
    }

    if (!verifyAppleOAuthState(state)) {
      setError("잘못된 인증 요청입니다. 다시 로그인해 주세요.");
      return;
    }

    handleAppleLogin(idToken);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [_hasHydrated, searchParams]);

  async function handleAppleLogin(idToken: string) {
    try {
      const result = await loginWithOidc({
        socialType: "apple",
        idToken,
      });

      if (cancelled.current) return;

      setTokens(result.accessToken, result.refreshToken);
      setProfileStatus(result.profileStatus);
      setTermsStatus(result.termsStatus);

      router.push("/");
    } catch (err) {
      if (cancelled.current) return;
      setError(toUserMessage(err));
    }
  }

  if (error) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        <p className="text-body-16 text-red">{error}</p>
        <button
          type="button"
          onClick={() => router.push("/login")}
          className="text-body-14 text-brand underline cursor-pointer"
        >
          로그인 페이지로 돌아가기
        </button>
      </div>
    );
  }

  return (
    <div className="flex min-h-screen items-center justify-center">
      <p className="text-body-16 text-text-secondary animate-pulse">
        Apple 로그인 처리 중...
      </p>
    </div>
  );
}

// ── 에러 메시지 매핑 ────────────────────────────────
function toUserMessage(err: unknown): string {
  const msg = typeof err === "string" ? err : err instanceof Error ? err.message : "";

  if (msg.includes("user_cancelled_authorize"))
    return "Apple 인증이 취소되었습니다.";
  if (msg.includes("missing_id_token"))
    return "인증 정보를 받지 못했습니다. 다시 시도해 주세요.";

  // 백엔드 에러 코드 매핑
  if (msg.includes("ID_TOKEN_SIGNATURE_VALIDATION_FAILED"))
    return "인증 토큰 검증에 실패했습니다. 다시 시도해 주세요.";
  if (msg.includes("ID_TOKEN_DECODING_FAILED"))
    return "인증 토큰을 처리할 수 없습니다. 다시 시도해 주세요.";
  if (msg.includes("NOT_SUPPORTED_SOCIAL_TYPE"))
    return "지원하지 않는 로그인 방식입니다.";

  // 네트워크/타임아웃
  if (msg.includes("네트워크") || msg.includes("시간이 초과"))
    return msg;

  return "로그인에 실패했습니다. 잠시 후 다시 시도해 주세요.";
}
